...
export function KidsClassOfferPopup() {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        className="w-full max-w-md sm:max-w-xl overflow-hidden rounded-[28px] border border-gray-200/80 dark:border-gray-800/80 bg-white/95 dark:bg-gray-900/95 p-0 shadow-2xl focus-visible:outline-none"
      >
        <div className="flex max-h-[min(90vh,640px)] flex-col">
          <div className="relative">
            <div
              className="absolute inset-x-0 top-0 z-20 flex justify-end px-3 sm:px-4"
              style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 0.5rem)" }}
            >
              <button
                className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-black/60 text-white backdrop-blur transition hover:bg-black/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            ...
            <div className="relative h-[220px] w-full overflow-hidden sm:h-64">
              <Image
                src="/banner.jpeg"
              ...
              <div className="absolute left-5 right-5 top-6 flex items-center gap-2 text-[0.65rem] font-semibold uppercase tracking-[0.35em] text-white/70 sm:left-6 sm:right-6 sm:text-xs">
                <Sparkles className="h-4 w-4" />
                Limited Time Offer
              </div>
              <div className="absolute bottom-6 left-5 right-5 space-y-2 text-white sm:left-6 sm:right-6">
                ...
                <DialogDescription className="text-sm text-white/85 sm:text-base">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto px-5 pb-6 pt-5 sm:px-8 sm:pb-8">
