  ...
  return (
    ...
    >
      ...
      <Container className="relative z-10 flex flex-col gap-12 py-14">
            ...
            <div className="space-y-5 text-sm text-slate-700 dark:text-white/70 sm:px-1 lg:px-2">
              <div className="flex items-center gap-4">
                <div className="relative h-12 w-40">
                  <Image src="/logo.svg" alt="Footloose" fill priority sizes="160px" className="object-contain" />
                </div>
              ...
              <p className="max-w-lg text-base leading-relaxed text-slate-700 dark:text-white/70">
                Footloose Edwin&apos;s Dance Company delivers expressive choreography, bespoke productions, and inspiring training programs that light up stages across India.
              </p>
              <div className="space-y-3 text-sm">
                <p className="font-medium text-slate-900 dark:text-white">Call</p>
                <Link
          ...
          <nav className="grid grid-cols-1 gap-10 text-sm text-slate-700 dark:text-white/70 sm:grid-cols-2 sm:gap-x-12 sm:gap-y-10 lg:grid-cols-2 lg:grid-rows-2 lg:gap-x-8 lg:gap-y-8 xl:gap-x-12 xl:gap-y-10 2xl:gap-x-16 2xl:gap-y-12">
            {linkGroups.map((group, groupIndex) => (
              <div
                ...
                style={{ 
                  animation: isVisible ? `fadeInUp 0.6s ease-out ${groupIndex * 0.1}s both` : 'none' 
                }}
              >
                ...
                <div className="flex flex-col gap-4 lg:gap-2.5 xl:gap-3">
                  {group.links.map((link, linkIndex) => (
                    <Link
                      ...
                      style={{ 
                        animation: isVisible ? `fadeInUp 0.5s ease-out ${(groupIndex * 0.15) + (linkIndex * 0.08)}s both` : 'none' 
                      }}
                    >
                      ...
                      <span className="relative flex-1 min-w-0 transition-colors duration-300 group-hover:text-slate-900 dark:group-hover:text-white whitespace-normal break-words hyphens-auto lg:leading-tight xl:leading-snug">{link.label}</span>
                      <span className="relative inline-flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/30 transition-all duration-300 group-hover:scale-110 group-hover:rotate-12 group-hover:shadow-xl group-hover:shadow-indigo-500/50 dark:from-indigo-400 dark:to-purple-500 dark:shadow-indigo-400/30 dark:group-hover:shadow-indigo-400/60 lg:h-7 lg:w-7 xl:h-8 xl:w-8 2xl:h-9 2xl:w-9">
                        <ArrowRight className="h-3.5 w-3.5 transition-all duration-300 group-hover:translate-x-0.5 lg:h-3.5 lg:w-3.5 xl:h-4 xl:w-4 2xl:h-4 2xl:w-4" strokeWidth={2.5} />
                      </span>
                    </Link>
                  ))}
                </div>
        ...
        <div className="flex flex-col gap-6 rounded-3xl border border-white/60 bg-white/70 px-6 py-6 shadow-lg shadow-indigo-200/30 backdrop-blur-xl transition-colors dark:border-white/10 dark:bg-white/5 dark:shadow-black/20 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-500 dark:text-white/60">
              Connect With Us
            </p>
            <p className="mt-1 text-sm text-slate-600 dark:text-white/70">
              Follow Footloose for behind-the-scenes moments, class drops, and live showcases.
            </p>
          </div>
